...
def dashboard():
    with get_db() as db:
        today_sales = db.execute(
            "SELECT COUNT(*) as count, COALESCE(SUM(total),0) as total FROM sales WHERE date(created_at)=date('now') AND status='completed'"
        ).fetchone()
        ...
        month_sales = db.execute(
            "SELECT COUNT(*) as count, COALESCE(SUM(total),0) as total FROM sales WHERE strftime('%Y-%m',created_at)=strftime('%Y-%m','now') AND status='completed'"
        ).fetchone()
        ...
        sales_by_day = db.execute(
            ...
            "FROM sales WHERE created_at >= datetime('now', '-7 days') AND status='completed' "
