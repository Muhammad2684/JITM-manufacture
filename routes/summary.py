...
@login_required
def summary():
    """Get financial summary with optional date filtering for income statement."""
    date_from = request.args.get('date_from', '')
    date_to = request.args.get('date_to', '')
    section = request.args.get('section', 'all')  # 'balance_sheet', 'income_statement', or 'all'
    
    with get_db() as db:
        result = {}
        
        # Balance Sheet (point-in-time, no date filtering)
        if section in ['all', 'balance_sheet']:
        ...
        # Income Statement (period-based, with date filtering)
        if section in ['all', 'income_statement']:
            date_filter = ''
            date_params = []
